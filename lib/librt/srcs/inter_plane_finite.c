...
/*   Updated: 2019/07/02 21:58:56 by nihuynh          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	inter_finite_rectangle(t_inter *data, t_plane *plane, float dist)
{
	...
	scale_x = vec3_dot(&orig_to_inter, &plane->x)
		/ vec3_dot(&plane->x, &plane->x);
	if (plane->size.x && fabsf(scale_x) > plane->size.x)
		return (0);
	scale_y = vec3_dot(&orig_to_inter, &plane->y)
		/ vec3_dot(&plane->y, &plane->y);
	return (!(plane->size.y && fabsf(scale_y) > plane->size.y));
}
	...
	inter_finite_disk(t_inter *data, t_plane *plane, float dist)
{
	...
	return (!(fabsf(scale) > plane->size.x));
}
