	...
	ui_sphere(void *ap, void *res)
{
	...
	ui_apply_rotation(&sphere->x, &sphere->y, &sphere->z);
	...
	ui_plane(void *app_v, void *res)
{
	...
	ui_apply_rotation(&plane->x, &plane->n, &plane->y);
	...
	ui_cylinder(void *app_v, void *res)
{
	...
	ui_apply_rotation(&cylinder->x, &cylinder->n, &cylinder->z);
	if (igInputFloat("Radius", &tmp.radius, 0, 0, "%g", 0))
	...
	ui_cone(void *app_v, void *res)
{
	...
	ui_apply_rotation(&cone->x, &cone->n, &cone->z);
