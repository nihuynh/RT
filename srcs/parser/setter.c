...
/*   Updated: 2019/05/18 00:53:18 by nihuynh          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	sphere_set(void *sphere, char **greed, int i)
{
	...
	if (sphere == NULL)
		ft_parse_err(__func__, __LINE__, ERR_PARSE_SET_SP, i, greed[i]);
	psphere = sphere;
	...
	plane_set(void *plane, char **greed, int i)
{
	...
	if (plane == NULL)
		ft_parse_err(__func__, __LINE__, ERR_PARSE_SET_PL, i, greed[i]);
	pplane = plane;
	...
	cylinder_set(void *cylinder, char **greed, int i)
{
	...
	if (cylinder == NULL)
		ft_parse_err(__func__, __LINE__, ERR_PARSE_SET_CY, i, greed[i]);
	pcylinder = cylinder;
	...
	cone_set(void *cone, char **greed, int i)
{
	...
	if (cone == NULL)
		ft_parse_err(__func__, __LINE__, ERR_PARSE_SET_CO, i, greed[i]);
	pcone = cone;
...
{
	if (light == NULL)
		ft_parse_err(__func__, __LINE__, ERR_PARSE_SET_LI, i, greed[i]);
